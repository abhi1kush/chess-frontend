...
import {
  ...
  type PGNAction,
} from '../../analysis/redux/analysisActions';

import { FromToSquare } from '../../../shared/types/AnalysisTypes';

import { createEmptyAnalysisData } from '../../analysis/services/analysisData';
      ...
      const analysisData = state.analysisData.map((row, i) =>
        i === index
          ? {
              evalScore,
              bestMove: bestMove ?? '',
              moveClassification: moveClassification ?? row.moveClassification ?? null,
            }
          : row
      );
