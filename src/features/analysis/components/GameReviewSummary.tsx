...
export default function GameReviewSummary({
  ...
  whitePlayerName,
  blackPlayerName,
}: GameReviewSummaryProps) : React.JSX.Element {
  const { whiteStat, blackStat } : {whiteStat : Record<string, number>, blackStat : Record<string, number>} = useMemo(
    () => aggregateGameReviewStats(analysisData, numMoves),
    [analysisData, numMoves],
  );

  const accWhite = useMemo(() => computePlayerAccuracy(whiteStat), [whiteStat]);
  const accBlack = useMemo(() => computePlayerAccuracy(blackStat), [blackStat]);

  const rows = useMemo(
    () =>
      GAME_REVIEW_ROWS.filter(
        (row) =>
          row.id !== 'unknown' || (whiteStat.unknown ?? 0) + (whiteStat.unknown ?? 0) > 0,
      ),
    [whiteStat, blackStat],
  );
    ...
    <section className="game-review-summary" aria-labelledby="game-review-heading">
      ...
      <div className="game-review-summary__scroll">
        <div className="game-review-summary__stats" role="table" aria-label="Move quality by category">
          {rows.map((row) => {
            const wc = whiteStat[row.id] ?? 0;
            const bc = blackStat[row.id] ?? 0;
            return (
              <div
                key={row.id}
