...
import React, { ReactNode, useEffect, useRef, useState, useCallback, Fragment } from 'react';
...
interface AnalysisDataItem { moveClassification?: string | null; evalScore?: number | null; bestMove?: string; }
...
interface MovesProps { 
  ...
  linePlies?: LinePly[]; exploreLine?: boolean; 
  lineCursor?: number; 
  onJumpToMainLine: (index: number) => void; 
  onEnterUserLine?: () => void; onBeginReview?: () => void; 
...
function renderMoveCell(
  { san, isWhite, moveIdx, reviewComplete, analysisData }:
  { san: string; isWhite: boolean; moveIdx: number; reviewComplete: boolean; analysisData: AnalysisDataItem[] }
): ReactNode | null {
 ...
 */
function formatUserLineWithNumbers(fenAtBranch: string, plies: LinePly[]): string {
