    ...
    readFileContent(file)
      .then((pgnData) => {
        try {
          ...
          const termination = chess.getHeaders().Termination;
          const blackPlayerName = chess.getHeaders().Black || '-';
          const whitePlayerName = chess.getHeaders().White || '-';
          ...
          moves.forEach((move) => {
            const moveDetails = chess.move(move);
            const promotionPiece : PieceSymbol | null | undefined = moveDetails.isPromotion() ? moveDetails.promotion : null;
            fromToSquares.push({
